...
import kotlinx.android.synthetic.main.non_teaching_update.view.*
import registration.LoginActivity

class NonteachersProfileActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_nonteachers_profile)
        ...
        documentReference.addSnapshotListener(this
            ...
            if (documentSnapshot != null) {
                ...
                Picasso.get().load(documentSnapshot.getString("imageUrl")).into(profile)
        ...
        Udetails.setOnClickListener{
            ...
            if(Name?.isBlank()!!){
                ...
                val view = LayoutInflater.from(this).inflate(R.layout.non_teaching_update, null)
                val dialog =
                ...
                view.ok3.setOnClickListener {
                    val Mobile = view.umobile3.text.toString()
                    val CCP = view.uccp3.selectedCountryCode
                    val Address = view.uaddress3.text.toString()
                    val City = view.ucity3.text.toString()
                    val State = view.ustate3.text.toString()
                    val Country = view.ucountry3.text.toString()
                    val Zipcode = view.uzipcode3.text.toString()
                    val Holding = view.uholding3.text.toString()
                    ...
                    Toast.makeText(
                        this@NonteachersProfileActivity,
                        "Information Updated",
                        Toast.LENGTH_SHORT
                    ).show()
                }
                view.exit3.setOnClickListener {
                    mAlertDialog.dismiss()
                }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when( item.itemId) {
            ...
            R.id.menu_signout ->{
                Auth.signOut()
                val intent = Intent(this@NonteachersProfileActivity, LoginActivity::class.java)
                startActivity(intent)
                finish()
            }
            android.R.id.home ->{
                val intent = Intent(this@NonteachersProfileActivity,
